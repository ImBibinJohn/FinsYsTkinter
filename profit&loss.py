...
def profitloss():
    ...
    def incomevalues():
        ...
        nme='Income'
        try:
            cursor.execute("SELECT name,balance FROM accounts1 WHERE acctype =%s",([nme]))
            val=cursor.fetchall()
            for i in val:
            ...
            pass
        try:
            cursor.execute("SELECT name,balance FROM accounts WHERE acctype =%s",([nme]))
            val=cursor.fetchall()
            for i in val:
                tk.Label(contframe,text=i[0],bg="#FFFFFF",font=('times new roman', 14)).place(relx=0.08,rely=x)
                tk.Label(contframe,text = i[1],bg="#FFFFFF",font=('times new roman', 12)).place(relx=0.86,rely=x)
    ...
    def costofgoodsvalue():
        ...
        y=x+0.08
        try:
            cursor.execute("SELECT name,balance FROM accounts1 WHERE acctype =%s",([nmee]))
            val=cursor.fetchall()
            ...
            pass
        try:
            cursor.execute("SELECT name,balance FROM accounts WHERE acctype =%s",([nmee]))
            val=cursor.fetchall()
            print(val)
            for i in val:
                tk.Label(contframe,text=i[0],bg="#FFFFFF",font=('times new roman', 14)).place(relx=0.08,rely=y)
